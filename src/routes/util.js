...
function divide(points) {
  var half = _.floor(points.orderedByX.length / 2),
    ...
    leftByY = getPointsOrderedByY(
      -Infinity,
      points.orderedByX[half].x,
      points.orderedByY
    ), // O(n)
    rightByX = _.slice(points.orderedByX, half),
    rightByY = getPointsOrderedByY(
      points.orderedByX[half].x,
      Infinity,
      points.orderedByY
    ); // O(n)
...
function getDistance(pair) {
  return Math.sqrt(
    Math.pow(pair[0].x - pair[1].x, 2) + Math.pow(pair[0].y - pair[1].y, 2)
  );
}
...
function combine(left, right, closestOnLeft, closestOnRight) {
  var closestPair = _.minBy([closestOnLeft, closestOnRight], getDistance),
    ...
    xInterval = {
      min: frontierPoint.x - minDist,
      max: frontierPoint.x + minDist
    },
...
module.exports = { fnc: closestPair.divideAndConquer };
